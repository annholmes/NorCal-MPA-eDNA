...
path <- "~/Desktop/CCFRP_2021/MiFish_field_MS/MiFish_field_MS_fastq"
...
taxa <- assignTaxonomy(seqtab.nochim, "~/Desktop/CCFRP_2021/MiFish_field_MS/MiFish_RDB.txt", multithread=TRUE, tryRC=TRUE)
...
write_xlsx(df, "~/Desktop/CCFRP_2021/MiFish_field_MS/MiFish_field_MS_tax.xlsx")

# phyloseq
metadata <- read.csv("MiFish_field_MS_map.csv")
...
plot_bar(ps.filt, x="sample_ID", fill="Species") +
  facet_wrap(~location~sample_type~site_type, nrow = 1, scales="free") +
  labs(x = "Sample ID", y = "Abundance")  +
  labs(title = "MiFish Field MS (filtered)") +
  theme(plot.title = element_text(color="blue", size=18, face="bold", hjust = 0.5))


# Sample type
ps.bh <- subset_samples_no_zero(ps.filt, location == "Bodega Head")
ps.bh.mpa <- subset_samples_no_zero(ps.bh, site_type == "MPA")
ps.bh.ref <- subset_samples_no_zero(ps.bh, site_type == "REF")

ps.sp <- subset_samples_no_zero(ps.filt, location == "Stewarts Point")
ps.sp.mpa <- subset_samples_no_zero(ps.sp, site_type == "MPA")
ps.sp.ref <- subset_samples_no_zero(ps.sp, site_type == "REF")

df <- as.data.frame(tax_table(ps.bh.mpa))
df$ASV <- rownames(df)
write_xlsx(df, "~/Desktop/CCFRP_2021/MiFish_field_MS/bh_mpa.xlsx")

df <- as.data.frame(tax_table(ps.bh.ref))
df$ASV <- rownames(df)
write_xlsx(df, "~/Desktop/CCFRP_2021/MiFish_field_MS/bh_ref.xlsx")

df <- as.data.frame(tax_table(ps.sp.mpa))
df$ASV <- rownames(df)
write_xlsx(df, "~/Desktop/CCFRP_2021/MiFish_field_MS/sp_mpa.xlsx")

df <- as.data.frame(tax_table(ps.sp.ref))
df$ASV <- rownames(df)
write_xlsx(df, "~/Desktop/CCFRP_2021/MiFish_field_MS/sp_ref.xlsx")

ps.nc <- subset_samples_no_zero(ps.filt, sample_type == "negative control")
df <- as.data.frame(tax_table(ps.nc))
df$ASV <- rownames(df)
write_xlsx(df, "~/Desktop/CCFRP_2021/MiFish_field_MS/nc.xlsx")