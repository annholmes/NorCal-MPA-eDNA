...
# Getting ready #
path <- "~/Desktop/CCFRP_2021/MiSebastes_tank/MiSebastes_tank_fastq"
fnFs <- sort(list.files(path, pattern = "_R1_001.fastq.gz", full.names = TRUE))
fnRs <- sort(list.files(path, pattern = "_R2_001.fastq.gz", full.names = TRUE))

# cutadapt #
FWD <- "AAGCTCATTCAAGTGCTT" # forward primer sequence
REV <- "GRCCACTYACACAATTCT" # reverse primer sequence
...
# RDB <- read.xlsx("MiSebastes_RDB.xlsx", 1)
# write.table(RDB, file = "MiSebastes_RDB.txt", sep = ";",
#             row.names = FALSE, col.names = FALSE, quote = FALSE)
# go to RDB_final-formatting.Rmd

# Assign taxonomy
taxa <- assignTaxonomy(seqtab.nochim, "~/Desktop/CCFRP_2021/MiSebastes_tank/MiSebastes_RDB.txt", multithread=TRUE, tryRC=TRUE)
...
write_xlsx(df, "~/Desktop/CCFRP_2021/MiSebastes_tank/MiSebastes_tank_tax12.xlsx")
...
# phyloseq #
metadata <- read.csv("MiSebastes_map.csv")
...
ps.nc <- subset_samples(ps.filt, site == "pcr")
...
plot_bar(ps.filt, x="sample_ID", fill="Species") +
  ...
  labs(title = "MiSebastes Tank (filtered)") +
